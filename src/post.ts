...
const cleanupRattlerBuildBin = () => {
  const rattlerBuildBinPath = options.rattlerBuildBinPath
  const rattlerBuildBinDir = path.dirname(rattlerBuildBinPath)
  core.debug(`Cleaning up rattler-build binary ${rattlerBuildBinPath}.`)
  return fs.rm(rattlerBuildBinPath).then(() => removeEmptyParentDirs(rattlerBuildBinDir))
}
